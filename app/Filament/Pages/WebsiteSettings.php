...
class WebsiteSettings extends AbstractPageSettings
{
    ...
    {
        return [
            'site_name' => config('app.name', 'F4K12'),
            'site_active' => true,
            'maintenance_message' => [
                'it' => 'Il nostro sito web è attualmente in manutenzione programmata. Riprova tra qualche minuto.',
                'en' => 'Our website is currently undergoing scheduled maintenance. Please check back soon.',
            ],
            // 'social_profiles' => [
            //     [
            //         'title' => 'Facebook',
            ...
            'footer_copyright' => [
                'it' => '© '.date('Y').' '.config('app.name', 'F4K12').'. All rights reserved.',
            ],
        ];
    }
                    ...
                    ->schema([
                        Grid::make()->schema([
                            TextInput::make('site_name')
                                ...
                                ->hintIconTooltip('website.site_name')
                                ->required(),
                            ToggleButtons::make('site_active')
                                ->label('Site active')
                                ->live()
                                ->inline()
                                ->boolean(trueLabel: 'Online', falseLabel: 'Maintenance mode')
                                ->default(true),
                        ]),
                        ...
                        Textarea::make('maintenance_message')
                            ...
                            ->hintIcon('heroicon-o-tag')
                            ->hintIconTooltip('website.maintenance_message')
                            ->rows(3)
                            ->translatableTabs()
                            ->visible(fn (Get $get) => ! $get('site_active')),

                        Section::make('Color')
                            ->columns(3)
                            ->schema([

                                Select::make('filament_colors.primary')
                                    ->label('Primary ')
                                ...
                                Select::make('filament_colors.success')
                                    ->label('Success')
                                    ->options($this->getColorOptions())
                                    ->allowHtml()
                                    ->searchable()
                                ...
                                Select::make('filament_colors.warning')
                                    ->label('Warning')
                                    ->options($this->getColorOptions())
                                    ->allowHtml()
                                    ->searchable()
                                ...
                                Select::make('filament_colors.danger')
                                    ->label('Danger')
                                    ->options($this->getColorOptions())
                                    ->allowHtml()
                                    ->searchable()
                                ...
                                Select::make('filament_colors.info')
                                    ->label('Info')
                                    ->options($this->getColorOptions())
                                    ->allowHtml()
                                    ->searchable()
                                ...
                                Select::make('filament_colors.gray')
                                    ->label('Gray')
                                    ->options($this->getColorOptions())
                                    ->allowHtml()
                                    ->searchable()
                    ...
                    ->tabs([
                        Tabs\Tab::make('Contacts')
                            ...
                            ->schema([
                                Grid::make()
                                    ...
                                    ->schema([
                                        Section::make('Sender')
                                            ->description('The address used as the sender for messages sent from contact forms.')
                                            ->schema([
                                                TextInput::make('mail_from_name')
                                                    ->hintIcon('heroicon-o-tag')
                                                    ->hintIconTooltip('website.mail_from_name')
                                                    ->label('Name')
                                                    ->required(),
                                                TextInput::make('mail_from_email')
                                                    ->hintIcon('heroicon-o-tag')
                                                    ->hintIconTooltip('website.mail_from_email')
                                                    ->label('Email')
                                                    ->required(),
                                            ])->columnSpan(1),
                                        Section::make('Recipient')
                                            ->description('The email address that will receive messages sent from contact forms.')
                                            ->schema([
                                                TextInput::make('mail_to_name')
                                                    ->hintIcon('heroicon-o-tag')
                                                    ->hintIconTooltip('website.mail_to_name')
                                                    ->label('Name')
                                                    ->required(),
                                                TextInput::make('mail_to_email')
                                                    ->hintIcon('heroicon-o-tag')
                                                    ->hintIconTooltip('website.mail_to_email')
                                                    ->label('Email')
                                                    ->required(),
                                            ])->columnSpan(1),
                                        Section::make('Reply-to Address')
                                            ->description('The email address to which users can reply.')
                                            ->schema([
                                                TextInput::make('reply_to_name')
                                                    ->hintIcon('heroicon-o-tag')
                                                    ->hintIconTooltip('website.reply_to_name')
                                                    ->label('Name')
                                                    ->required(),
                                                TextInput::make('reply_to_email')
                                                    ->hintIcon('heroicon-o-tag')
                                                    ->hintIconTooltip('website.reply_to_email')
                                                    ->label('Email')
                                                    ->required(),
                                            ])->columnSpan(1),
                                    ]),
                                ...
                                Repeater::make('social_profiles')
                                    ...
                                    ->schema([
                                        ...
                                        FileUpload::make('icon')
                                            ...
                                            ->helperText('Recommended size: 32x32px, Format: png')
                                        ...
                                        Text::make(function (Get $get) {
                                            ...
                                            return new HtmlString('<div><strong>Preview: </strong></div><div>'.$this->sanitizeSvg($svg).'</div>');
                                        })->visible(fn (Get $get) => $get('use_svg')),
                        ...
                        Tabs\Tab::make('Altro')
                            ...
                            ->schema([
                                ...
                                Grid::make()
                                    ->schema([
                                        RichEditor::make('footer_col_1')
                                            ->label('Footer Column 1')
                                            ->hintIcon('heroicon-o-tag')
                                            ->hintIconTooltip('website.footer_col_1')
                                            ->translatableTabs()
                                            ->extraAttributes(fn () => is_multilingual() ? [] : ['class' => 'hide-tabs']),
                                        RichEditor::make('footer_col_2')
                                ...
                                Textarea::make('footer_copyright')
                                    ...
                                    ->hintIcon('heroicon-o-tag')
                                    ->hintIconTooltip('website.footer_copyright')
                                    ->translatableTabs()
                                    ->extraAttributes(fn () => is_multilingual() ? [] : ['class' => 'hide-tabs']),
                            ]),
                    ])
    ...
    {
        // Sanitize SVGs in social profiles
        if (isset($this->data['social_profiles']) && is_array($this->data['social_profiles'])) {
            foreach ($this->data['social_profiles'] as $index => $profile) {
                if (! empty($profile['svg'])) {
                    $this->data['social_profiles'][$index]['svg'] = $this->sanitizeSvg($profile['svg']);
                }
            }
        }
        ...
        // Refresh the current page after saving so the UI reflects persisted values.
        redirect(request()->header('Referer'));
    }

    /**
     * Build select options with a small color swatch for each palette.
     *
    ...
    protected function getColorOptions(): array
    {
        ...
        foreach (Color::all() as $slug => $palette) {
            // Try to pick a mid-tone (500) or fallback to the first shade available
            $shade = $palette[500] ?? array_values($palette)[0] ?? null;

            // Convert OKLCH to rgb for inline style if necessary
            $rgb = is_string($shade) ? Color::convertToRgb($shade) : '';

            $label = ucfirst($slug);

            $swatch = '';
            if (! empty($rgb)) {
                $swatch = "<span class=\"inline-block size-8 rounded-lg my-1.5 align-middle mr-2.5\" style=\"background:{$rgb};border:1px solid rgba(0,0,0,0.12);box-shadow:0 0 0 1px rgba(255,255,255,0.02) inset;\"></span>";
            }

            $options[$slug] = $swatch."<span class=\"align-middle\">{$label}</span>";
        }

        return $options;
    }

    protected function sanitizeSvg(string $svg): string|false
    {
        return (new \enshrined\svgSanitize\Sanitizer)->sanitize($svg);
    }

    /**
     * Returns a hint text showing the number of characters used and remaining.
     */
