...
<x-layouts.main>
    <article>
        {{-- Header --}}
        <x-cms.posts.header :category="$post->category">
            <x-slot:title>
                {{ $post->title }}
            </x-slot>
        ...
        @if ($image && $post->extras['show_featured_image'])
            <x-cms.featured-image-cover :image_url="$image->getUrl()" />
        @endif

        {{-- Contenuto --}}
        {{-- Usiamo il componente esistente che renderizza il contenuto del post --}}
        <x-rich-content :model="$post" />
    </article>
</x-layouts.main>
